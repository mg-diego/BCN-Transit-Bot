...
class UserDataManager:
    ...
    def __init__(
        self, spreadsheet_name: str = "TMB", credentials_file: str = "credentials.json"
    ):
        ...
        try:
            ...
            logger.info(
                f"Connected to Google Spreadsheet '{spreadsheet_name}' successfully."
            )
    ...
    def _load_users(self, force_refresh=False):
        if not force_refresh and self._users_cache["data"] is not None:
            if datetime.now() - self._users_cache["timestamp"] < timedelta(
        ...
        for user in users:
            if "receive_notifications" in user:
                user["receive_notifications"] = str(
    ...
    def _load_favorites(self, force_refresh=False):
        if not force_refresh and self._favorites_cache["data"] is not None:
            if datetime.now() - self._favorites_cache["timestamp"] < timedelta(
                seconds=self.CACHE_TTL
            ):
    ...
    def _load_searches(self, force_refresh=False):
        if not force_refresh and self._searches_cache["data"] is not None:
            if datetime.now() - self._searches_cache["timestamp"] < timedelta(
                seconds=self.CACHE_TTL
            ):
    ...
    def _load_alerts(self, force_refresh=False):
        if not force_refresh and self._alerts_cache["data"] is not None:
            if datetime.now() - self._alerts_cache["timestamp"] < timedelta(
                seconds=self.CACHE_TTL
            ):
    ...
    def register_user(self, user_id: int, username: str):
        ...
        self.users_ws.append_row(
            [user_id, username, now, now, 1, "en", True, json.dumps([])]
        )
        self._users_cache["data"].append(
            {
                ...
                "already_notified": json.dumps(
                    [e.__dict__ for e in []], ensure_ascii=False
                ),
            }
        )
        return 1
    ...
    def update_notified_alerts(self, user_id, alert_id):
        ...
        for idx, user in enumerate(users, start=2):
            if str(user["user_id"]) == str(user_id):
                ...
                self.users_ws.update_cell(
                    idx, self.USERS_ALREADY_NOTIFIED, json.dumps(already_notified)
                )
                self._users_cache["data"][idx - 2][
                    "already_notified"
                ] = already_notified
                return True
        return False
    ...
    def remove_deprecated_notified_alert(self, user_id, alert_id):
        logger.debug(
            f"Removing deprecated notified alerts for user_id={user_id} -> '{alert_id}'"
        )
        users = self._load_users()
        for idx, user in enumerate(users, start=2):
            if str(user["user_id"]) == str(user_id):
    ...
    def update_user_receive_notifications(self, user_id, value: bool):
        ...
        logger.debug(
            f"Updating receive_notifications for user_id={user_id} to '{value}'"
        )
        users = self._load_users()
        for idx, user in enumerate(users, start=2):
            if str(user["user_id"]) == str(user_id):
                self.users_ws.update_cell(
                    idx, self.USERS_RECEIVE_NOTIFICATIONS_INDEX, bool_value
                )
    ...
    def row_to_user(self, row: List[str]) -> User:
        ...
        return User(
            ...
            initial_start=datetime.strptime(
                row.get("initial_start"), "%Y:%m:%d %H:%M:%S"
            ),
    ...
    def remove_favorite(self, user_id: int, type: str, item_id: str):
        ...
        logger.debug(
            f"Removing favorite: user_id={user_id}, type={type}, item_id={item_id}"
        )
    ...
    def get_favorites_by_user(self, user_id: int):
        ...
        return sorted(
            user_fav, key=lambda f: self.FAVORITE_TYPE_ORDER.get(f.get("type"), 999)
        )
    ...
    def register_search(self, type: str, line: str, code: str, name: str):
        ...
        logger.debug(
            f"Registering search: type={type}, line={line}, code={code}, name={name}"
        )
        ...
        for idx, search in enumerate(searches, start=2):
            if (
                str(search.get("type")) == str(type)
                and str(search.get("code")) == str(code)
                and str(search.get("line")) == str(line)
            ):
                new_uses = int(search.get("searches", 0)) + 1
                self.searches_ws.update_cell(
                    idx, self.SEARCHES_LAST_SEARCH_COLUMN_INDEX, now
                )
                self.searches_ws.update_cell(
                    idx, self.SEARCHES_USES_COLUMN_INDEX, new_uses
                )
    ...
    def register_alert(self, transport_type: TransportType, api_alert: Alert):
        ...
        for idx, ws_alert in enumerate(ws_alerts, start=2):
            if str(ws_alert.get("type")) == str(transport_type.value) and str(
                ws_alert.get("id")
            ) == str(api_alert.id):
                logger.debug(
                    f"Alert already registered: type={transport_type}, alert_id={api_alert.id}"
                )
                return False

        self.alerts_ws.append_row(
            [
                api_alert.id,
                transport_type.value,
                api_alert.begin_date.isoformat() if api_alert.begin_date else "",
                api_alert.end_date.isoformat() if api_alert.end_date else "",
                api_alert.status,
                api_alert.cause,
                json.dumps(
                    [pub.__dict__ for pub in api_alert.publications], ensure_ascii=False
                ),
                json.dumps(
                    [ent.__dict__ for ent in api_alert.affected_entities],
                    ensure_ascii=False,
                ),
            ]
        )
        ...
        logger.info(
            f"New alert registered: type={transport_type}, alert_id={api_alert.id}"
        )
        return True
    ...
    def remove_alert(self, alert: Alert) -> bool:
        ...
        """
        logger.debug(
            f"Removing alert: type={alert.transport_type}, alert_id={alert.id}"
        )

        ws_alerts = self._load_alerts()

        for idx, ws_alert in enumerate(
            ws_alerts, start=2
        ):  # start=2 porque la fila 1 es encabezado
            if (
                ws_alert.get("type")
                and str(ws_alert.get("type")) == str(alert.transport_type.value)
                and str(ws_alert.get("id")) == str(alert.id)
            ):
                self.alerts_ws.delete_rows(idx)
                logger.info(
                    f"Alert removed from spreadsheet: type={alert.transport_type}, alert_id={alert.id}"
                )

                if self._alerts_cache["data"] is not None:
                    self._alerts_cache["data"] = [
                        a
                        for a in self._alerts_cache["data"]
                        if not (
                            str(a.get("transport_type")) == str(alert.transport_type)
                            and str(a.get("id")) == str(alert.id)
                        )
                    ]
                return True

        logger.warning(
            f"Alert not found: type={alert.transport_type}, alert_id={alert.id}"
        )
        return False
    ...
    def row_to_alert(self, row: Dict[str, str]) -> Alert:
        ...
        try:
            ...
            affected_entities = [
                AffectedEntity(**ent)
                for ent in json.loads(row.get("affected_entities", "[]"))
            ]

            begin_date = (
                datetime.fromisoformat(row.get("begin_date"))
                if row.get("begin_date")
                else None
            )
            end_date = (
                datetime.fromisoformat(row.get("end_date"))
                if row.get("end_date")
                else None
            )
            ...
            return Alert(
                id=row.get("id"),
                transport_type=(
                    TransportType(str(row.get("type").lower()))
                    if row.get("type")
                    else None
                ),
