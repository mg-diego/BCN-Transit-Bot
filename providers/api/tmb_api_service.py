...
from domain.bus import BusLine, BusLineRoute, BusStop, NextBus, create_bus_stop
...
class TmbApiService:
    ...
    async def _get(self, endpoint: str, params: dict = None):
        ...
        merged_params = {"app_id": self.app_id, "app_key": self.app_key}
        if params:
    ...
    async def fetch_transit_items(
        self, url: str, model_class, *, filter_fn=None, sort_key=None, factory_fn=None
    ):
    ...
    async def get_bus_lines(self) -> List[BusLine]:
        ...
        items, _ = await self.fetch_transit_items(
            url, BusLine, sort_key=self._natural_key
        )
        return items
    ...
    async def get_metro_lines(self) -> List[MetroLine]:
        ...
        items, _ = await self.fetch_transit_items(
            url, MetroLine, sort_key=lambda x: x.NOM_LINIA
        )
    ...
    async def get_station_connections(self, metro_station_id):
        ...
        return [
            c for c in connections if "FM" not in c.NOM_LINIA
        ]  # Filter out Funicular
