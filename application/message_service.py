...
class MessageService:
    ...
    async def handle_interaction(
        ...
        if update.callback_query:
            logger.info(
                f"[{self.__class__.__name__}] Handling callback_query for user {self.get_user_id(update)}"
            )
            return await self.edit_inline_message(
                update, text, reply_markup, parse_mode
            )
        elif update.message:
            logger.info(
                f"[{self.__class__.__name__}] Handling message for user {self.get_user_id(update)}"
            )
    ...
    async def send_new_message(
        ...
        logger.info(
            f"[{self.__class__.__name__}] Sent new message {msg.message_id} to user {self.get_user_id(update)}"
        )
        return msg

    async def send_new_message_from_bot(
        self, bot, user_id, text, parse_mode=ParseMode.HTML
    ):
    ...
    async def edit_inline_message(
        ...
        self._cache_message(update, msg)
        logger.info(
            f"[{self.__class__.__name__}] Edited inline message {msg.message_id} for user {self.get_user_id(update)}"
        )
        return msg
    ...
    async def send_message_direct(
        ...
        logger.info(
            f"[{self.__class__.__name__}] Sent direct message {msg.message_id} to chat {chat_id}"
        )
        return msg
    ...
    async def edit_message_by_id(
        ...
        logger.info(
            f"[{self.__class__.__name__}] Edited message {message_id} in chat {chat_id}"
        )

    async def send_new_message_from_callback(
        self, update: Update, text: str, reply_markup=None, parse_mode=ParseMode.HTML
    ):
        """
        ...
        self._cache_message(update, msg)
        logger.info(
            f"[{self.__class__.__name__}] Sent new callback message {msg.message_id} to user {self.get_user_id(update)}"
        )
        return msg

    async def send_location(
        self, update: Update, latitude: str, longitude: str, reply_markup=None
    ):
        """
        ...
        self._cache_message(update, msg)
        logger.info(
            f"[{self.__class__.__name__}] Sent location to user {self.get_user_id(update)}: ({latitude}, {longitude})"
        )
        return msg

    async def send_map_image(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, line_name
    ):
    ...
    def _cache_message(self, update, msg):
        ...
        logger.debug(
            f"[{self.__class__.__name__}] Cached message {msg.message_id} for user {user_id}"
        )
        ...
        if user_id in self._user_messages:
            for msg_id in self._user_messages[user_id]:
                try:
                    ...
                    logger.info(
                        f"[{self.__class__.__name__}] Deleted message {msg_id} for user {user_id}"
                    )
                except Exception as e:
                    logger.warning(
                        f"[{self.__class__.__name__}] Error deleting message {msg_id}: {e}"
                    )
