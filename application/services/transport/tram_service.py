...
class TramService(ServiceBase):
    ...
    async def get_stop_routes(self, stop_id: int) -> List[LineRoute]:
        start = time.perf_counter()
        stop = await self.get_stop_by_id(stop_id)
        routes = await self._get_from_cache_or_api(
            f"tram_routes_{stop_id}",
            lambda: self.tram_api_service.get_next_trams_at_stop(stop.outboundCode, stop.returnCode),
            cache_ttl=30,
        )
        elapsed = (time.perf_counter() - start)
        logger.info(f"[{self.__class__.__name__}] get_stop_routes({stop_id}) -> {len(routes)} routes (tiempo: {elapsed:.4f} s)")
        return routes
