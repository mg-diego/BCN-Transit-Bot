...
class BicingService(ServiceBase):
    ...
    def __init__(
        self, bicing_api_service: BicingApiService, cache_service: CacheService = None
    ):
    ...
    async def get_stations_by_name(self, station_name) -> List[BicingStation]:
        ...
        return self.fuzzy_search(
            query=station_name, items=stations, key=lambda stop: stop.streetName
        )

    async def get_station_by_id(self, station_id) -> BicingStation:
        stations = await self.get_all_stations()
        filtered_stations = [
            station for station in stations if int(station_id) == int(station.id)
        ]
