...
class MetroService(ServiceBase):
    ...
    async def get_all_lines(self) -> List[MetroLine]:
        ...
        cached_lines = await self._get_from_cache_or_data(
            static_key, None, cache_ttl=3600 * 24
        )
        cached_alerts = await self._get_from_cache_or_data(
            alerts_key, None, cache_ttl=3600
        )
    ...
    async def get_stations_by_line(self, line_id) -> List[MetroStation]:
        ...
        cached_stations = await self._get_from_cache_or_data(
            cache_key, None, cache_ttl=3600 * 24
        )
        if cached_stations:
        ...
        for api_station in api_stations:
            connections = await self.tmb_api_service.get_station_connections(
                api_station.CODI_ESTACIO
            )
        ...
        return await self._get_from_cache_or_data(
            cache_key, line_stations, cache_ttl=3600 * 24
        )
    ...
    async def get_stations_by_name(self, station_name) -> List[MetroStation]:
        ...
        logger.debug(
            f"[{self.__class__.__name__}] get_stations_by_name({station_name})"
        )
        return self.fuzzy_search(
            query=station_name, items=stations, key=lambda stop: stop.NOM_ESTACIO
        )
    ...
    async def get_station_by_id(self, station_id) -> MetroStation:
        ...
        logger.debug(
            f"[{self.__class__.__name__}] get_station_by_id({station_id}) -> {station}"
        )
        return station
    ...
    async def get_line_by_name(self, line_name):
        lines = await self.get_all_lines()
        line = next(
            (l for l in lines if str(l.ORIGINAL_NOM_LINIA) == str(line_name)), None
        )
        logger.debug(
            f"[{self.__class__.__name__}] get_line_by_name({line_name}) -> {line}"
        )
        return line
    ...
    async def _build_and_cache_static_stations(self):
        ...
        for line in lines:
            line_stations = await self.tmb_api_service.get_stations_by_metro_line(
                line.CODI_LINIA
            )
