...
class FgcHandler(HandlerBase):
    ...
    async def ask_search_method(
        ...
        await super().ask_search_method(
            update, context, transport_type=TransportType.FGC
        )
    ...
    async def show_station(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user_id, chat_id, line_id, fgc_station_id = (
            self.message_service.extract_context(update, context)
        )
        logger.info(
            f"Showing station info for user {user_id}, line {line_id}, stop {fgc_station_id}"
        )
        ...
        await self.fgc_service.get_station_routes(
            fgc_station.name, fgc_station.moute_id, line_id
        )
        await self.update_manager.stop_loading(update, context)

        async def update_text():
            next_fgc = await self.fgc_service.get_station_routes(
                fgc_station.name, fgc_station.moute_id, line_id
            )
            is_fav = self.user_data_manager.has_favorite(
                user_id, TransportType.FGC.value, fgc_station_id
            )
            text = (
        ...
        self.start_update_loop(
            ...
            chat_id,
            message.message_id,
            get_text_callable=update_text,
            previous_callback=default_callback,
        )
        logger.info(
            f"Started update loop task for user {user_id}, station {fgc_station_id}"
        )
