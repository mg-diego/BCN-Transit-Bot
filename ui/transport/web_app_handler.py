...
class WebAppHandler:
    ...
    def web_app_data_router(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.info(
                f"[{self.__class__.__name__}] Received WebApp payload: {payload}"
            )
        ...
        logger.info(
            f"[{self.__class__.__name__}] Routing WebApp data of type: {data_type}"
        )
        ...
        if data_type == TransportType.BUS.value:
            ...
            logger.warning(
                f"[{self.__class__.__name__}] Unrecognized WebApp data type: {data_type}"
            )
