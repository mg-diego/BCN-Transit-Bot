...
class ReplyHandler:
    ...
    async def handle_reply_from_user(
        self,
        update: Update,
        ...
        if user_location is None:
            await message_service.send_new_message(
                update,
                language_manager.t("results.location.ask"),
                keyboard_factory.location_keyboard(),
            )
            self.current_search = str(update.message.text)
        ...
        if len(self.current_search) < 3:
            await update.message.reply_text(
                language_manager.t("results.minimum.letters")
            )
            return

        message = await update_manager.start_loading(
            update, context, language_manager.t("results.searching")
        )
        ...
        if not only_bicing:
            if self.current_search.isdigit():  # ONLY FOR BUS STOPS AND BICING
                ...
                metro_stations = await metro_service.get_stations_by_name(
                    self.current_search
                )
                bus_stops = await bus_service.get_stops_by_name(self.current_search)
                rodalies_stations = await rodalies_service.get_stations_by_name(
                    self.current_search
                )
                bicing_stations = await bicing_service.get_stations_by_name(
                    self.current_search
                )
        ...
        stops_with_distance = DistanceHelper.build_stops_list(
            metro_stations,
            bus_stops,
            tram_stops,
            rodalies_stations,
        ...
        metro_count = len(
            [s for s in stops_with_distance if s["type"] == TransportType.METRO.value]
        )
        bus_count = len(
            [s for s in stops_with_distance if s["type"] == TransportType.BUS.value]
        )
        tram_count = len(
            [s for s in stops_with_distance if s["type"] == TransportType.TRAM.value]
        )
        rodalies_count = len(
            [
                s
                for s in stops_with_distance
                if s["type"] == TransportType.RODALIES.value
            ]
        )
        bicing_count = len(
            [s for s in stops_with_distance if s["type"] == TransportType.BICING.value]
        )
        ...
        if not only_bicing:
            ...
            encoded = self.mapper.map_bicing_stations(
                near_bicing_stations, user_location.latitude, user_location.longitude
            )
            await message_service.send_new_message(
                update,
                language_manager.t("results.location.received"),
                keyboard_factory.map_reply_menu(encoded),
            )
    ...
    async def location_handler(self, update, context):
        if self.previous_search == "🚴 Bicing":
            await self.handle_reply_from_user(
                update, context, update.message.location, only_bicing=True
            )
